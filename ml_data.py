...
#We establish functions in a separate file.
...
#We'll get our data
#Also seperate it into our target and train data.
...
for indx in X_train.index:
    ...
    for word in words:
        if word in word_score_train.keys():
            word_score_train[word].append(rating)
        else:
            word_score_train[word] = [rating]
...
word_score_series_train = pd.Series(word_score_train).apply(average_rating).dropna().sort_values(ascending=False)

word_score_series_train_copy = word_score_series_train.copy()
for word, rating in word_score_series_train_copy.items():
    if rating < 2.5 or rating >= 4:
        pass
    else:
        word_score_series_train_copy = word_score_series_train_copy.drop(index = word)
...
for index in X_train.index:
    ...
    new_words = unrated_value_drop(data,word_score_series_train_copy)
